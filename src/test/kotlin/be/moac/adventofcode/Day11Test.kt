...
import be.moac.adventofcode.Direction.*
...
internal class Day11Test {
    ...
    @Test
    fun `that direction can turn`() {

        //Expect:
        SoftAssertions.assertSoftly { softly ->
            softly.assertThat(Up turn 0).isEqualTo(Left)
            softly.assertThat(Up turn 1).isEqualTo(Right)

            softly.assertThat(Left turn 0).isEqualTo(Down)
            softly.assertThat(Left turn 1).isEqualTo(Up)

            softly.assertThat(Down turn 0).isEqualTo(Right)
            softly.assertThat(Down turn 1).isEqualTo(Left)

            softly.assertThat(Right turn 0).isEqualTo(Up)
            softly.assertThat(Right turn 1).isEqualTo(Down)
        }

    }

    @Test
    fun `that direction can step one`() {

        //Expect:
        SoftAssertions.assertSoftly { softly ->
            softly.assertThat((5 to 5) step Up).isEqualTo(5 to 4)
            softly.assertThat((5 to 5) step Left).isEqualTo(4 to 5)
            softly.assertThat((5 to 5) step Down).isEqualTo(5 to 6)
            softly.assertThat((5 to 5) step Right).isEqualTo(6 to 5)
        }
    ...
    fun `run robot`() {
        ...
        DrawingRobot(instructions = instructions,
            ship = Array(500) { CharArray(500) {'.'} }.apply { this[250][250] = '#' },
            startPosition = 250 to 250).run()

        assertThat(DrawingRobot(instructions).run()).isEqualTo(2594)
